<template>
    ...
    <Head title="Factory List" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Factory List</h2>
        </template>
        <template #link>
            <Link :href="route('factories.create')"
        ...
        <div class="p-4 bg-white rounded-lg shadow-xs">
            <div class="w-full mb-8 overflow-hidden border rounded-lg shadow-sm dark:border-gray-600">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr
                                ...
                                <th scope="col" class="px-6 py-3">Factory Name</th>
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-600 dark:bg-gray-700">
                            <tr v-for="factory in factories.data" :key="factory.id"
                                ...
                                <td class="px-6 py-3 text-sm"> {{ factory.id }}</td>
                                <td class="px-6 py-3 text-sm"> {{ factory.factory_name }}</td>
                                <td class="px-6 py-3 text-sm"> {{ factory.location }}</td>
                                <td class="px-6 py-3 text-sm"> {{ factory.email }}</td>
                                <td class="px-6 py-3 text-sm"> {{ factory.website }}</td>
                                <td class="inline-flex gap-2 px-6 py-3 text-sm align-middle">
                                    <Link :href="route('factories.show', factory.id)"
                                    ...
                                    <Link :href="route('factories.edit', factory.id)"
                                    ...
                                    <button @click="confirmDeleteAdmin(factory.id)"
                ...
                <div
                    ...
                    <Pagination :links="factories.links" :current-page="factories.current_page"
                        :items-per-page="itemsPerPage" :total-items="factories.total" @updateItemsPerPage="fetchData" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
...
const confirmDeleteAdmin = (factoryId) => {
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            ...
            axios.delete(route('factories.destroy', factoryId), {
                headers: {
                ...
                .catch(error => {
                    console.error('Error deleting factory:', error.response ? error.response.data : error.message);
